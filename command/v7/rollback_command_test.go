	...
	When("the first revision is set as the rollback target", func() {
		...
		When("the app has at least one revision", func() {
			BeforeEach(func() {
				...
				// 	v7action.Revisions{
				// 		ccv3.Revision{Version: 1, GUID: "some-1-guid"},
				// 		ccv3.Revision{Version: 2, GUID: "some-2-guid"},
				// 	},
			...
			It("successfully executes the command and outputs warnings", func() {
				...
				// Expect(fakeActor.CreateDeploymentByApplicationAndRevisionCallCount()).To(Equal(1), "call count of CreateDeploymentByApplicationAndRevision")
				// _, revisionGUID := fakeActor.CreateDeploymentByApplicationAndRevisionArgsForCall(0)
				// Expect(revisionGUID).To(Equal("some-2-guid"))
